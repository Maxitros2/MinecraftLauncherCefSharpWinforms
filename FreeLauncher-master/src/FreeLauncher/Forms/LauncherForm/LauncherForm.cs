...
namespace FreeLauncher.Forms
{
    public partial class LauncherForm : RadForm
    {
        ...
        public LauncherForm(Configuration configuration)
        {
            ...
            AppendLog(new string('=', 12));
            //this.FormElement.TitleBar.ForeColor = System.Drawing.Color.Red;
            this.FormElement.TitleBar.BackColor = Color.FromArgb(30, 42, 80);
            this.FormElement.Border.ForeColor= Color.FromArgb(30, 42, 80);
            this.FormElement.Border.BackColor= Color.FromArgb(30, 42, 80);
            this.Text = "Mad";
            this.FormElement.TitleBar.BorderPrimitive.ForeColor= Color.FromArgb(30, 42, 80);

            this.FormElement.TitleBar.TitleBarFill.BackColor = Color.Transparent;
            this.FormElement.BackColor= Color.FromArgb(30, 42, 80);
            this.radDropDownList2.ListElement.HighlightColor = Color.FromArgb(30, 42, 80);
            this.radDropDownList2.ListElement.EnableHighlight = false;
            this.radDropDownList2.VisualListItemFormatting += (o1, args)
                  =>
              {
                  
                      args.VisualItem.NumberOfColors = 1;
                      args.VisualItem.BackColor = Color.FromArgb(58, 64, 104);
                 

              };
            this.radDropDownList3.ListElement.BackColor = Color.FromArgb(30, 42, 80);
            this.radDropDownList3.ListElement.ForeColor = Color.FromArgb(30, 42, 80);
            this.radDropDownList3.ListElement.HighlightColor = Color.FromArgb(58, 64, 104);
            this.radDropDownList3.ListElement.EnableHighlight = false;
            this.radDropDownList3.VisualListItemFormatting += (o1, args)
                  =>
            {
            ...
            foreach(RadPageViewStripItem item in this.radPageView1.ViewElement.Items)
            {
                ...
                Brush brush = new SolidBrush(Color.FromArgb(57, 74, 127));
                ...
                if (!item.IsSelected)
                {
                    item.ForeColor = Color.FromArgb(58, 64, 104);
                }
                else
                {
                    Bitmap bm = new Bitmap(200, 100);
                    Graphics g = Graphics.FromImage(bm);
                    g.FillRectangle(brush, 0, 90, 200, 10);
                    item.BackgroundImage = bm;
                   
                    //   item.BackColor = Color.FromArgb(255, 132, 0);
                }
                item.MouseEnter += (i1, i2)
                      =>
                  {
                      ((RadPageViewStripItem)i1).ForeColor = Color.White;
                  };
                item.MouseLeave += (i1, i2) 

                       =>
                {
                    if(!((RadPageViewStripItem)i1).IsSelected)
                        ((RadPageViewStripItem)i1).ForeColor = Color.FromArgb(58, 64, 104);
                };
                item.MouseLeave += (i1, i2)
                      =>
                {
                        ...
                        =>
                {
                    ...
                    //((RadPageViewStripItem)i1).BackColor= Color.FromArgb(255, 132, 0);
                    Bitmap bm = new Bitmap(200, 100);
                    Graphics g = Graphics.FromImage(bm);
                    g.FillRectangle(brush, 0, 90, 200, 10);
                    
                    ((RadPageViewStripItem)i1).BackgroundImage = bm;

                    foreach (RadPageViewStripItem item2 in this.radPageView1.ViewElement.Items)
                    {
                        if (item2 != i1)
                        {
                            item2.ForeColor = Color.FromArgb(58, 64, 104);
                            bm = new Bitmap(10,10);
                            item2.BackgroundImage = bm;
...
<i>{ProductName} {ProductVersion}</i>
    ...
    {
        private readonly MadForm _launcherForm;
        ...
        public MinecraftProcess(Process minecraftProcess, MadForm launcherForm, Profile profile)
        {
